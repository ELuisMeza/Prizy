...
Cualquier servicio de hosting estático puede servir los archivos de la carpeta `dist/`:
...
Actualmente la aplicación no requiere variables de entorno ya que utiliza datos estáticos en JSON. Si en el futuro se requiere una API backend, las variables se configurarían en el servicio de hosting.
...
Obtiene productos filtrados por nombre, categoría y subcategoría.

**Parámetros**:
- `productName: string` - Término de búsqueda
- `category: string | null` - Nombre de categoría (opcional)
- `subcategory: string | null` - Nombre de subcategoría (opcional)
- `limit?: number` - Límite de resultados (opcional)

**Retorna**: `Promise<TypeProduct[]>`

**Ejemplo**:
```typescript
const products = await productsService.getProducts('iPhone 15', 'Tecnología', 'Telefonía', 10);
```
...
**Retorna**: `Promise<TypeProduct | undefined>`

#### `productsService.getSimilarProducts()`

Obtiene productos similares basados en categoría y subcategoría.

**Parámetros**:
- `currentProductId: string` - ID del producto actual
- `category: string` - Categoría
- `subcategory: string` - Subcategoría
- `limit?: number` - Límite de resultados (opcional, default: 4)
...
Actualmente el proyecto no incluye tests automatizados. Para agregar testing se recomienda:
...
- **Equipo Prizy** - Desarrollo inicial y mantenimiento

## 🙏 Agradecimientos

- **Chakra UI** - Por la excelente biblioteca de componentes
- **Vite** - Por la herramienta de construcción ultra rápida
- **React** - Por el ecosistema robusto
- **Chart.js** - Por las capacidades de visualización de datos
